...
class CreatePromotionUrlUuidServiceTest {
    ...
    @InjectMocks
    CreatePromotionUrlUuidService createPromotionUrlUuidService;

    @Mock
    PromotionUrlUuidRepository promotionUrlUuidRepository;

    @DisplayName("UUID를 생성하고 저장한다")
    @Test
    void create() {
        // given
        PromotionUrlUuid savedEntity = PromotionUrlUuid.builder()
                .uuid(UUID.randomUUID())
                .build();
        
        when(promotionUrlUuidRepository.save(any(PromotionUrlUuid.class)))
