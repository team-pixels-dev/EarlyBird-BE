...
class CreateClickLogServiceTest {
    ...
    @Mock
    private UserClickLogCountRepository userClickLogCountRepository;

    @Mock
    private UserClickLogCount userClickLogCount;

    @InjectMocks
    private CreateClickLogService service;
    ...
    void createLogWithInfoLevel(CapturedOutput output) throws Exception {
        ...
        CreateClickLogServiceRequest request = CreateClickLogServiceRequest.builder()
                .clickTime(clickTime)
                .clientId(clientId)
                .clickType(clickType)
                .build();

        when(userClickLogCountRepository.findByClientIdAndClickTypeAndClickDate(any(), any(), any())).thenReturn(Optional.of(userClickLogCount));
        ...
        assertThat(output.getOut()).contains("\"click-time\":\"" + clickTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")) +"\"");
        assertThat(output.getOut()).contains("Click 이벤트 발생 [click-type: "+ clickType + ", client-id: "+ clientId + "]");
    ...
    @DisplayName("UserClickLogCountRepository 에 요청과 관련된 정보가 저장되어 있지 않다면 새로운 UserClickLogCount 객체를 만든다")
    @Test
    void saveNewUserClickLogCount() {
        ...
        CreateClickLogServiceRequest request = CreateClickLogServiceRequest.builder()
                ...
                .clickType(clickType)
                .build();
        when(userClickLogCountRepository.findByClientIdAndClickTypeAndClickDate(any(), any(), any())).thenReturn(Optional.empty());
        when(userClickLogCountRepository.save(any(UserClickLogCount.class))).thenReturn(userClickLogCount);
