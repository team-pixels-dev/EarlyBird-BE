...
public class CreateClickLogService {
    ...
    private void createLog(CreateClickLogServiceRequest request) {
        Map<String, String> logAttributes  = Map.of(
                "event-type", "client-click",
                "click-type", request.getClickType(),
                "client-id", request.getClientId(),
                "click-time", request.getClickTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))
        );
        String logMessage = "Click 이벤트 발생 [click-type: {}, client-id: {}]";
        LogUtil.log(LogLevel.INFO, logAttributes, logMessage, request.getClickType(), request.getClientId());
    }

    private void updateClickLogStats(CreateClickLogServiceRequest request) {
        UserClickLogCount userClickLogCount = userClickLogCountRepository
                .findByClientIdAndClickTypeAndClickDate(
                        request.getClientId(), request.getClickType(), request.getClickTime().toLocalDate())
