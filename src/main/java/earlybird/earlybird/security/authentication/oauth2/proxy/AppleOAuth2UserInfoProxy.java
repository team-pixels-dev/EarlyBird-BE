 ...
 * <h3>애플 로그인 과정</h3>
 *
 * 1. 클라이언트에서 전송한 Authorization Code로 Client Secret JWT 토큰 생성 <br>
 * 2. 애플 서버의 Client Secret 검증 API 호출 <br>
...
public class AppleOAuth2UserInfoProxy implements OAuth2UserInfoProxy {
    ...
    private String createClientSecret(String appleAuthorizationCode) {
        ...
        return Jwts.builder()
                ...
                .claim(
                        ...
                        Date.from(
                                now.plusSeconds(
                                        expiredMs))) // exp: client secret이 만료되는 시간 또는 그 이후를 식별함
    ...
    private PrivateKey loadAppleLoginP8Key() {
        ...
        try (PEMParser pemParser =
                new PEMParser(new InputStreamReader(resource.getInputStream()))) {
