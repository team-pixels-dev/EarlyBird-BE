...
public class SendPromotionEmailService {
    ...
    public void sendVerificationEmail(SendVerificationEmailServiceRequest request) {
         ...
         *  - 이메일 유효성 검사 (완)
         *  - URL을 구성할 UUID 발급 (완)
         *  - DB에서 애플 프로모션 URL 하나 조회
         *      - 락을 걸었는데 이거 성능 문제 좀 더 고민 필요
         *  - 이메일로 인증 URL 전송 (여기에 UUID가 들어감)
         *      - 비동기 처리 필요 (완)
         *      - no-reply@earlybirdteam.com 이메일 적용 필요
         *  - 인증 URL을 클릭하면 애플 프로모션 URL로 리다이렉션 (완)
         *      - 엣지 케이스 좀 더 고민 필요
         *  - 스프링 트랜잭션에 대한 고민 필요
         *  - Retry 로직 도입 고려 (완)
        ...
        String promotionUrl = getApplePromotionUrlService.getPromotionUrl(promotionCampaign.getId()).getPromotionUrl();
        ...
        PromotionEmailVerification promotionEmailVerification =
                createPromotionEmailVerification(
                        promotionCampaign, promotionUrlUuid, request.getEmail(), emailPromotionCodeIssuance);
    ...
    private void checkEmailAddress(SendVerificationEmailServiceRequest request) {
        Boolean isValidDomain = checkEmailAddressService
                .checkValidPromotionEmail(request.getEmail(), request.getPromotionCampaignId());
    ...
    private EmailPromotionCodeIssuance createEmailPromotionCodeIssuance(
        ...
        EmailPromotionCodeIssuance emailPromotionCodeIssuance = EmailPromotionCodeIssuance.builder()
    ...
    private PromotionEmailVerification createPromotionEmailVerification(
        ...
        PromotionEmailVerification promotionEmailVerification = PromotionEmailVerification.builder()
                .promotionCampaign(promotionCampaign)
                .promotionUrlUuid(promotionUrlUuid)
